...
def results(res:requests.Response):    
    ...
    title = [re.sub(pattern="/movie/|hd|watch|[0-9]*",repl='',string=" ".join(i.split("-"))) for i in urls]#.replace("/movie/",'').replace('hd','').replace('watch',"")|re.sub(pattern="-",repl=" ",string=parse(
    ids = [i.split('-')[-1] for i in urls]
    return [list(sublist) for sublist in zip(urls,title,ids)]
...
def play(obj):os.system(f'mpv --referrer="https://sflix.se" {obj}')#{obj["sources"][0]["file"]}
