...
class WebScraper:
    ...
    async def search(self, q:str=None) -> str:
        return NotImplementedError()
    
    def results(self, data: str) -> list:
        return NotImplementedError()
    
    def dl(self, url: str, name: str, path: str = determine_path(), subtitle: str = None):#"./"
        if not subtitle:
            os.system(f'ffmpeg -loglevel error -stats -i "{url}" -c copy "{path}/{name}.mp4"')
            return print(self.blue(f"Downloaded {name} at {path}"))
        os.system(f'ffmpeg -loglevel error -stats -i "{url}" -i {subtitle} -c copy -c:s mov_text"{path}/{name}.mp4"')
    ...
    def play(self, url: str, name: str):
        try:
            try:
                os.system(f'mpv --referrer="{self.base_url}" "{url}" --force-media-title="mov-cli:{name}"')
            except ModuleNotFoundError: ##why do you even exist if you don't have MPV installed? WHY?
                os.system(f'vlc --http-referrer="{self.base_url}" "{url}" --meta-title="mov-cli{name}"')
