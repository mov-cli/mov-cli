...
class kinox(WebScraper):
        ...
        if "<b>File not found, sorry!</b>" not in html:
            ...
            if v:
                ...
                if vurl:
                    ...
                    url = self.client.head(vurl.group(1), redirects=False).headers.get(
                        "location"
                    )
                    return url
        raise Exception("Video not found or removed")
    ...
    def ask(self, url):
        ...
        hostlist = self.client.get(
            f"{self.base_url}/aGET/MirrorByEpisode/{rel}&Season={season}&Episode={option}"
        ).text
        name = re.findall("\/Stream\/(.*)\.", url)[0]
        if re.search("Hoster_92", hostlist):
            url = self.voe(
                f"{self.base_url}/aGET/Mirror/{name}&Hoster=92&Season={season}&Episode={option}"
            )
        elif re.search("Hoster_95", hostlist):
            url = self.doodstream(
                f"{self.base_url}/aGET/Mirror/{name}&Hoster=95&Season={season}&Episode={option}"
            )
        else:
            url = self.streamz(
