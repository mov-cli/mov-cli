...
class Sflix(Actvid):
    ...
    def cdn_url(self, rabbid, rose, num):
        ...
        #    'file']
        data = json.loads(self.client.get(
            f"https://rabbitstream.net/ajax/embed-4/getSources?id={rabbid}&_token={rose}&_number={num}").text)['sources'][0][
            'file']
        return data
    ...
    def ask(self, series_id):
        ...
        rf = self.client.get(f"{self.base_url}/ajax/v2/season/episodes/{season_ids[int(season) - 1]}")
    ...
    def ep_server_id(self, ep_id):
        rem = self.client.get(f"{self.base_url}/ajax/v2/episode/servers/{ep_id}/#servers-list")
        soup = BS(rem, 'html.parser')
        return [i['data-id'] for i in soup.select('.link-item')][0]