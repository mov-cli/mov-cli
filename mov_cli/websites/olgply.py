...
class OlgPly(WebScraper):
    ...
    def search(self, q: str = None) -> list:
        q = input(self.blue("[!] Please Enter the name of the Movie: ")) if q is None else q
    ...
    def cdn_url(self, name):
        ...
        req = httpx.get(f'https://olgply.com/api/?imdb={imdb_id}', headers={'User-Agent': 'Mozilla/5.0 (X11; Linux '
                                                                                          'x86_64; rv:100.0) '
                                                                                          'Gecko/20100101 '
                                                                                          'Firefox/100.0'}).text
    ...
    def cdn_url_ws(self, season, episode, name):
        imdb_id = get_imdb_id(name)
        print(f'https://olgply.com/api/?imdb={imdb_id}&season={season}&episode={episode}')
        req = httpx.get(f'https://olgply.com/api/?imdb={imdb_id}&season={season}&episode={episode}', headers={'User-Agent': 'Mozilla'
                                                                                                             '/5.0 ('
                                                                                                             'X11; '
                                                                                                             'Linux '
                                                                                                             'x86_64; '
    ...
    def ask(self, tmdb_id, name):
        ...
        season = input(self.lmagenta(f"Please input the season number(total seasons:{seasons}): "))
        episodes = get_season_episodes(tmdb_id, name, season)
        episode = input(self.lmagenta(f"Please input the episode number(total episodes:{episodes}): "))
        ...
        while choice not in range(len(result) + 1):
            ...
            if choice == 'q':
                ...
                try:
                    ...
                    print(self.red(f"[!]  This Episode / Movie is coming soon! | "), self.lmagenta(str(e)))
                    sys.exit(2)
            else:
