...
class Actvid(WebScraper):
    def __init__(self, base_url) -> None:
        ...
        self.rep_key = '6LfV6aAaAAAAAC-irCKNuIS5Nf5ocl5r0K3Q0cdz' #Google Recaptcha key
        self.rab_domain = x('https://rabbitstream.net:443') #encoding and then decoding the url
        #self.redo()
    ...
    def auth_token(self, key: str) -> str:
        ...
        r = self.client.get(f"https://www.google.com/recaptcha/api.js?render={self.rep_key}")
        s = r.text.replace("/* PLEASE DO NOT COPY AND PASTE THIS CODE. */", "")
        s = s.split(";")
        vtoken = s[10].replace("po.src=", "").split("/")[-2]
        r = self.client.get(f"https://www.google.com/recaptcha/api2/anchor?ar=1&hl=en&size=invisible&cb=xxmovclix&k={key}&co={self.rab_domain}&v={vtoken}").text
        ...
        return json.loads(self.client.post(f"https://www.google.com/recaptcha/api2/reload?k={key}", data=data).text.replace(")]}'", ''))
    ...
    def ask(self, series_id) -> str:
        ...
        season = input(self.lmagenta(f"Please input the season number(total seasons:{len(season_ids)}): "))
        ...
        episode = episodes[int(input(self.lmagenta(
            f"Please input the episode number(total episodes in season:{season}):{len(episodes)} : "))) - 1]
        return episode
    
    def cdn_url(self, rabbid, rose, num) -> str:
        self.client.set_headers({'X-Requested-With': 'XMLHttpRequest'})
        data = self.client.get(
            f"https://mzzcloud.life/ajax/embed-4/getSources?id={rabbid}&_token={rose}&_number={num}").json()['sources'][0][
            'file']
        return data

    def display(self, result:list):
        for ix, vl in enumerate(result): print(self.green(f"[{ix + 1}] {vl[2]} {vl[0]}"), end="\n\n")
        ...
        while choice not in range(len(result) + 1):
            ...
            if choice == 'q':
                ...
                try:
                    mov = result[int(input(self.yellow("[!] Please enter the number of the movie you want to download: "))) - 1]
    ...
    def ep_server_id(self, ep_id) -> str:
        req = self.client.get(f"{self.base_url}/ajax/v2/episode/servers/{ep_id}/#servers-list")
        soup = BS(req, 'html.parser')
        return [i['data-id'] for i in soup.select('.nav-item > a')][0]

    def get_link(self, thing_id):
        req = self.client.get(f"{self.base_url}/ajax/get_link/{thing_id}").json()['link']
        print(req)
