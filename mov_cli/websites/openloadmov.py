...
class openloadmov(WebScraper):
    ...
    def results(self, data: str) -> list:
        ...
        mov_or_tv = ["MOVIE" if items[i].find("span").text.__contains__("Movie") else "TV" for i in range(len(items))]
    ...
    def cdn_url(self, url):
        ...
        awd = self.client.post(self.getvid.format(vidhash), data={"hash": vidhash, "r": self.base_url + "/"}).json()["securedLink"]
        print(awd)
        return awd
